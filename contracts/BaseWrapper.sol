...
abstract contract BaseWrapper {
        ...
        returns (uint256 balance)
    {
        ...
        for (uint256 id = 0; id < vaults.length; id++) {
            balance =
                balance +
                ((vaults[id].balanceOf(account) * vaults[id].pricePerShare()) /
    ...
    function _withdraw(
            ...
            if (availableShares > 0) {
                ...
                if (amount != WITHDRAW_EVERYTHING) {
                    ...
                    uint256 estimatedShares = ((amount - withdrawn) *
                        10**uint256(vaults[id].decimals())) / // NOTE: Changes every iteration
        ...
        if (
            withdrawn > amount &&
            withdrawn - amount >
            _bestVault.pricePerShare() / 10**_bestVault.decimals()
        ) {
